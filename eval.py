...
class Metrics:
    ...
    def update(self, pred, target, name):
        ...
        # print(mean(abs(pred-target)), pred.mean(), target.mean())
        # import time 
        # time.sleep(1)
...
pred_img_name = sorted(glob(os.path.join(pred_path, "*", "*.png")))
# pred_img_name = glob(os.path.join(pred_path, "*", "*.png"))
# gt_img_name = glob(os.path.join("results_lt", "*", "*.png"))
gt_img_name = sorted(glob(os.path.join(args.mirrormask, "*", "SegmentationClassPNG", "*.png")))
