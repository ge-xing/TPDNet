...
def segformer(pretrained=True, num_labels=1):
    # id2label = {0: "others"}
    id2label = {k: str(k) for k in range(num_labels)}
...
class SegFormer(nn.Module):
    def __init__(self, pretrianed=True, num_labels=1, *args, **kwargs) -> None:
        super().__init__(*args, **kwargs)
        self.model = segformer(pretrianed, num_labels=num_labels)
...
class SegformerEncoder(nn.Module):
    def __init__(self, config: SegformerConfig):
        ...
        # self.prompt_generator = PromptGenerator(scale_factor=scale_factor, embed_dims=config.hidden_sizes,
        #                                 tuning_stage="1234",
    ...
    def forward(
        ...
        # if self.prompt_generator.handcrafted_tune:
        #     handcrafted1, handcrafted2, handcrafted3, handcrafted4 = self.prompt_generator.init_handcrafted(hidden_states)
        # else:
        #     handcrafted1, handcrafted2, handcrafted3, handcrafted4 = None, None, None, None

        # handcrafted = [handcrafted1, handcrafted2, handcrafted3, handcrafted4]
        for idx, x in enumerate(zip(self.patch_embeddings, self.block, self.layer_norm)):
            embedding_layer, block_layer, norm_layer = x
            # first, obtain patch embeddings
            hidden_states, height, width = embedding_layer(hidden_states)
            # need to plug in the embedding adapter feature
            # if f'{idx+1}' in self.prompt_generator.tuning_stage:
            #     prompt1 = self.prompt_generator.init_prompt(hidden_states, handcrafted[idx], idx+1)

            # second, send embeddings through blocks
            for i, blk in enumerate(block_layer):

                ## before the operator of blk, we need to plug the adapter features
                # if f'{idx+1}' in self.prompt_generator.tuning_stage:
                    # print(f"hidden_states shape is {hidden_states.shape}")
                    # print(prompt1[0].shape, prompt1[1].shape)
                    # hidden_states = self.prompt_generator.get_prompt(hidden_states, prompt1, idx+1, i)
