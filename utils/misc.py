...
def fscore(test, reference, beta_square=0.3,):
    """(1 + b^2) * TP / ((1 + b^2) * TP + b^2 * FN + FP)"""

    precision_ = cal_precision(test, reference)
    recall_ = cal_recall(test, reference)

    return (1 + beta_square) * precision_ * recall_ +  0.00001 /\
        (beta_square * precision_ + recall_ + 0.00001)


def cal_precision(pred, target):
    return metric.binary.precision(pred, target)

def cal_recall(pred, target):
    return metric.binary.recall(pred, target)
